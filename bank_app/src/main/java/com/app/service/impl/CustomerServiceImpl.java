...
public class CustomerServiceImpl implements CustomerService {
	...
	/*
	 * We want to make sure that all of the details are valid before entering
	 * into the database.
	 */
	@Override
	public boolean validateCustomerInfo(Customer c) throws BusinessException {
		...
		if (!CustInfoValidation.isValidCustomerName(c.getfName())) {
			throw new BusinessException(
					"Entered first name " + c.getfName() + " is not valid.");
		}
		if (!CustInfoValidation.isValidCustomerName(c.getfName())) {
			throw new BusinessException(
					"Entered last name " + c.getlName() + " is not valid.");
		}
		if (!CustInfoValidation
				.isValidAge((int) ((new Date().getTime() - c.getDob().getTime())
						/ (31556952000L)))) {

			throw new BusinessException(
					"Entered date " + c.getDob() + " is not valid.");
		}

		if (!CustInfoValidation.isTaxIdValid(c.getSsn())) {
			throw new BusinessException("Entered social security number "
					+ c.getSsn() + " is not valid.");
		}

		if (!CustInfoValidation.isValidContactNumber(c.getContactNum())) {
			throw new BusinessException("Entered contact number "
					+ c.getContactNum() + " is not valid.");
		}

		if (!CustInfoValidation.isValidEmail(c.getEmail())) {
			throw new BusinessException(
					"Entered last name " + c.getEmail() + " is not valid.");
		}

		if (!CustInfoValidation.isValidZip(c.getZip())) {
			throw new BusinessException(
					"Entered last name " + c.getZip() + " is not valid.");
		}
		
		/*
		 * return true if everything checks out. The method will fail if one of
		 * the details is invalid
		 */
		return true;
	}
	...
	public boolean validateCustomerId(int id) throws BusinessException {
		...
		if(!CustInfoValidation.isValidCustomerId(id)) {
			throw new BusinessException(
				"Entered customer id number " + id + " is not valid.");			
		}
		/*
		 * return true if everything checks out. The method will fail if one of
		 * the details is invalid
